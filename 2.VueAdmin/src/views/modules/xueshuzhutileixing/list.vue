		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"padding":"0","margin":"0 0 20px","borderRadius":"8px","flexWrap":"wrap","background":"none","display":"flex","width":"100%"}' :inline="true" :model="searchForm">
				<el-row :style='{"width":"auto","margin":"20px 20px 20px 0","display":"inline-block"}' >
					<div :style='{"margin":"0 5px 0 0","display":"inline-block"}'>
						<label :style='{"margin":"0 5px 0 0","color":"#000","display":"inline-block","lineHeight":"40px","fontSize":"14px","fontWeight":"500","height":"40px"}' class="item-label">学术主题类型</label>
						<el-input v-model="searchForm.xueshuzhutileixing" placeholder="学术主题类型" @keydown.enter.native="search()" clearable></el-input>
					</div>
				...
				<el-row class="actions" :style='{"width":"auto","margin":"20px 0","flexWrap":"wrap","display":"flex"}'>
					<el-button class="add" v-if="isAuth('xueshuzhutileixing','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont " :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","height":"40px"}'></span>
						添加
					</el-button>
					<el-button class="del" v-if="isAuth('xueshuzhutileixing','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
						<span class="icon iconfont " :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","height":"40px"}'></span>
						删除
					</el-button>
			...
			<div :style='{"width":"100%","padding":"0","overflow":"hidden","borderRadius":"8px"}'>
				<el-table class="tables"
					...
					:border='true'
					v-if="isAuth('xueshuzhutileixing','查看')"
					:data="dataList"
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='true'  
						prop="xueshuzhutileixing"
						label="学术主题类型">
						<template slot-scope="scope">
							{{scope.row.xueshuzhutileixing}}
						</template>
					</el-table-column>
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('xueshuzhutileixing','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan2" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="edit" v-if=" isAuth('xueshuzhutileixing','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('xueshuzhutileixing','删除') " type="primary" @click="deleteHandler(scope.row.id )">
...
import AddOrUpdate from "./add-or-update";
    ...
    getDataList() {
      ...
      let params = {
        ...
        order: 'desc',
           ...
           if(this.searchForm.xueshuzhutileixing!='' && this.searchForm.xueshuzhutileixing!=undefined){
            params['xueshuzhutileixing'] = '%' + this.searchForm.xueshuzhutileixing + '%'
          }
			let user = JSON.parse(this.$storage.getObj('userForm'))
			console.log(user)
			this.$http({
				url: "xueshuzhutileixing/page",
				method: "get",
				params: params
			}).then(({ data }) => {
				if (data && data.code === 0) {
	...
	xueshuzhutileixingstatusChange(e,row){
		if(row.status==0){
			row.passwordwrongnum = 0
		}
		this.$http({
			url:'xueshuzhutileixing/update',
			method:'post',
			data:row
		}).then(res=>{
			if(row.status==1){
