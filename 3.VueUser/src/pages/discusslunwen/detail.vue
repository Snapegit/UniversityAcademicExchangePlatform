...
<div>
	<div :style='{"padding":"20px 0","margin":"0px auto","borderColor":"#ddd","borderRadius":"0px","background":"none","borderWidth":"0 0 0px","width":"86%","borderStyle":"solid"}' class="breadcrumb-preview">
		<el-breadcrumb :separator="'>'" :style='{"fontSize":"14px","lineHeight":"1","justifyContent":"flex-start","display":"flex"}'>
			...
			<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" to="/index/discusslunwen"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="attr" :style='{"padding":"0","margin":"0","flexWrap":"wrap","background":"none","flex":"1","display":"flex","width":"48%","position":"relative","justifyContent":"space-between","order":"2"}'>
			...
			<div class="info" :style='{"minHeight":"460px","padding":"0","margin":"0","borderRadius":"0px","flexWrap":"wrap","background":"#dff4ff","display":"flex","width":"100%","justifyContent":"space-between","order":"2"}'>
				...
				<div class="btn" :style='{"width":"100%","padding":"0 20px","flexWrap":"wrap","display":"flex"}'>
					<el-button :style='{"border":"0px solid #009cf530","cursor":"pointer","padding":"0 10px","margin":"0 5px 0 0","outline":"none","color":"#fff","borderRadius":"2px","background":"#009cf5","width":"auto","lineHeight":"36px","fontSize":"14px","height":"36px"}' v-if="btnAuth('discusslunwen','修改')" @click="editClick">修改</el-button>
					<el-button :style='{"border":"0px solid #f5340030","cursor":"pointer","padding":"0 10px","margin":"0 5px 0 0","outline":"none","color":"#fff","borderRadius":"2px","background":"#f53400","width":"auto","lineHeight":"36px","fontSize":"14px","height":"36px"}' v-if="btnAuth('discusslunwen','删除')" @click="delClick">删除</el-button>
					<el-button :style='{"border":"0px solid #f5340030","cursor":"pointer","padding":"0 10px","margin":"0 5px 0 0","outline":"none","color":"#fff","borderRadius":"2px","background":"#f53400","width":"auto","lineHeight":"36px","fontSize":"14px","height":"36px"}' v-if="btnAuth('discusslunwen','私聊')&&detail.id!=mid" @click="chatClick">联系TA</el-button>
				</div>
			</div>
		</div>
    ...
    methods: {
      ...
      nextClick(page) {
		...
		backClick(){
			...
			this.$router.push({path: '/index/discusslunwen', query: params});
		},
		// 下载
		download(file){
			if(!file) {
      ...
      getDiscussList(page) {
        this.$http.get('discussdiscusslunwen/list', {params: {page, limit: this.pageSize, refid: this.detail.id}}).then(res => {
          if (res.data.code == 0) {
	  ...
	  discussDel(id){
		  this.$confirm('是否删除此评论？')
		    .then(_ => {
		      this.$http.post('discussdiscusslunwen/delete',[id]).then(res=>{
      ...
      submitForm(formName) {
        this.$refs[formName].validate((valid) => {
          if (valid) {
            ...
            this.$http.post('discussdiscusslunwen/add', this.form).then(res => {
              if (res.data.code == 0) {
                this.form.content = '';
	  ...
	  addDiscussNum(type){
		  ...
		  this.$http.post('discusslunwen/update',this.detail).then(res=>{
		...
		// 修改
		editClick(){
			this.$router.push(`/index/discusslunwenAdd?type=edit&&id=${this.detail.id}`);
		},
		// 删除
		async delClick(){
			await this.$confirm('是否删除此论文评论表？')
			  .then(_ => {
			    this.$http.post('discusslunwen/delete', [this.detail.id]).then(async res => {
					if (res.data.code == 0) {
						this.$message({
							type: 'success',
							message: '删除成功!',
